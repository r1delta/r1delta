...
namespace r1delta::materialsystem_dx11
{
		...
		TextureUploadCopyRange copyRange)
	{
		...
		try {
			...
			for (const TextureInitialDataEntry& entry : upload->entries) {
				const std::size_t entryBytes = entry.sysMemSlicePitch;
			...
			for (std::size_t i = 0; i < subresourceCount; ++i) {
				const std::size_t entryBytes = upload->entries[i].sysMemSlicePitch;
