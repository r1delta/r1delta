...
inline void* MemAlloc_AllocAligned(size_t nSize) {
	return MemAlloc_AllocAligned(nSize, 16);
}

inline void* MemAlloc_AllocAlignedFileLine(size_t nSize, const char* pFileName, int nLine) {
	return MemAlloc_AllocAlignedFileLine(nSize, 16, pFileName, nLine);
}
// Helper macro for converting the __FILE__ macro to a wide string
#define WIDEN2(x) L##x
...
inline Vector& AllocTempVector()
{
	...
	static std::atomic<int> s_nIndex(0);
	int nIndex;

	for (;;)
	{
		int nOldIndex = s_nIndex.load(std::memory_order_relaxed);
		nIndex = ((nOldIndex + 0x10001) & 0x7F);
		if (s_nIndex.compare_exchange_weak(nOldIndex, nIndex, std::memory_order_release, std::memory_order_relaxed))
		{
			break;
		}
