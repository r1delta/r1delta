...
extern "C"
{
  ...
  {
      __asm {
          mov    rax, rbp            // Move rbp to rax for comparison 
          cmp    eax, 0x250          // Compare value with 0x250
          jg     jump_to_ret0        // Jump if greater
          jmp    jump_to_continue    // Otherwise continue
      jump_to_ret0:
          jmp    [CNetChan__ProcessSubChannelData_ret0]
      jump_to_continue:
...
// TODO: fix after implementing discord client
// why would you need jwt for that
AuthResponse Server_AuthCallback(bool loopback, const char* serverIP, const char* token) {
	AuthResponse response;
	response.success = true;
	strncpy(response.failureReason, "JWT Auth Disabled", sizeof(response.failureReason));
	strncpy(response.discordName, "TestUser", sizeof(response.discordName));
	strncpy(response.pomeloName, "TestPomelo", sizeof(response.pomeloName));
	response.discordId = 123456789;

	// Original JWT logic commented out:
	/*
	if (token == nullptr) {
	...
	int a11)
{
	...
	if (result && result->status == 200) {
		try {
			...
			std::string pomeloName = "";
			std::string id = "";
			
			if (jsonResponse.contains("pomelo_name")) {
				pomeloName = jsonResponse["pomelo_name"].get<std::string>();
			} else {
				pomeloName = "R1Delta Player";
			}
			
			if (jsonResponse.contains("discord_id")) {
				id = jsonResponse["discord_id"].get<std::string>();
			}
...
do_engine(const LDR_DLL_NOTIFICATION_DATA* notification_data)
{
	...
	*(unsigned char*)((uint64_t)jmp_pos + 4) = 0x00;
	...
	VirtualProtect(allign, 14, PAGE_EXECUTE_READWRITE, &old_protect);
	*((unsigned char*)allign) = 0xFF;
	*(unsigned char*)((uintptr_t)allign + 1) = 0x25;
	*(unsigned char*)((uintptr_t)allign + 2) = 0x00;
	*(unsigned char*)((uintptr_t)allign + 3) = 0x00;
	*(unsigned char*)((uintptr_t)allign + 4) = 0x00;
	*(unsigned char*)((uintptr_t)allign + 5) = 0x00;
	*(uintptr_t*)((uintptr_t)allign + 6) = (uintptr_t)_CNetChan__ProcessSubChannelData_AsmConductBufferSizeCheck;
	VirtualProtect(allign, 14, old_protect, &old_protect);
}
